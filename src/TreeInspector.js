...
class TreeInspector {
	...
	this.options = {
	    hint: "root",
	    autoParse: true,
	    globalsRoot: globalThis,
	    maxDepth: Infinity,

	    // path inference bounds (for non-string roots)
	    inferDepth: 6,
	...
	// absolute identity of the *rootRef* (best effort)
	this._absRootParts = null; // e.g. ["window","lib","foo"]
	this._absRootPath  = null; // e.g. "window.lib.foo"

	this.tree = null;

	// indexes
	this.index = {
	    byPath: new Map(),   // absPath -> node
	    byRef: new WeakMap() // ref -> canonical node
	};

	// resolve input now
	const resolved = this._resolveInput(obj);
	this.rootRef       = resolved.ref;
	this._absRootParts = resolved.absParts;
	this._absRootPath  = resolved.absPath;

	// parse root label: last segment (or hint fallback)
	this.options.hint = resolved.hint ?? this.options.hint;

	if (this.options.autoParse !== false) {
	    this.parse({ maxDepth: this.options.maxDepth });
	}
    }
